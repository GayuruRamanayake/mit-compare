...
MIN_LENGTH_FOR_SEMANTIC_MATCH = 40
MIN_LENGTH_FOR_CONFIDENT_MATCH = 15


def align_clauses(original: list[dict], revised: list[dict], match_threshold: float = 60.0) -> list[dict]:
    used_revised_idx = set()
    results = []
    ...
    for orig_idx, orig in enumerate(original):
        orig_text = orig["text"]
        best_idx, best_score = None, 0.0
        for i, rev in enumerate(revised):
            if i in used_revised_idx:
                continue
            score = fuzz.ratio(orig_text, rev["text"])
            if score > best_score:
                best_score, best_idx = score, i

        effective_threshold = match_threshold
        if len(orig_text) < MIN_LENGTH_FOR_CONFIDENT_MATCH:
            effective_threshold = 90.0

        if best_idx is not None and best_score >= effective_threshold:
            used_revised_idx.add(best_idx)
            rev = revised[best_idx]
            status = "unchanged" if orig_text.strip() == rev["text"].strip() else "modified"
            results.append({
                "clause_id": f"c{orig_idx}",
                "status": status,
                "original_text": orig_text,
                "revised_text": rev["text"],
                "original_index": orig_idx,
                "revised_index": best_idx,
                "similarity": round(best_score, 1),
                "match_method": "fuzzy",
                # both sides' track record, always — nothing discarded
                "authors_original": orig["authors"],
                "authors_revised": rev["authors"],
                "comments_original": orig.get("comments", []),
                "comments_revised": rev.get("comments", []),
            })
        else:
            results.append({
                "clause_id": f"c{orig_idx}",
    ...
    if deleted and added:
        ...
        for i, del_clause in enumerate(deleted):
            ...
            if best_j is not None and best_sim >= SEMANTIC_MATCH_THRESHOLD:
                ...
                del_clause["similarity"] = round(best_sim * 100, 1)
                del_clause["match_method"] = "semantic"
                # del_clause already has authors_original/comments_original correct;
                # pull in the revised side from the merged "added" clause
                del_clause["authors_revised"] = add_clause["authors_revised"]
                del_clause["comments_revised"] = add_clause["comments_revised"]
                add_clause["_merged"] = True
